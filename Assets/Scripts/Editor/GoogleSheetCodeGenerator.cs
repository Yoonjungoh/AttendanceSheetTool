...
public class GoogleSheetCodeGenerator : EditorWindow
{
    ...
    private void GenerateAllCode()
    {
        ...
        Log("  Managers.Start()");
        Log("    → InitAsync()");
        Log("      → SpecData.DownloadDataSheetAsync()");
        foreach (var name in _results.Keys)
            if (name != "Config") Log($"        → Fetch{name}Async()");
        Log("      → Config.DownloadConfigAsync()");
        Log("");
        Log("[ 사용 예시 ]");
        foreach (var name in _results.Keys)
    ...
    private void GenerateEnumFile()
    {
        ...
        // enum 이름별로 이를 참조하는 모든 (시트, 컬럼) 쌍을 먼저 모은다.
        // 같은 enum을 여러 시트가 enum(TypeName)으로 공유 참조할 수 있으므로,
        // 한 시트만 스캔하면 다른 시트에만 존재하는 멤버를 놓칠 수 있다.
        var enumRefs = new Dictionary<string, List<(SheetParseResult sheet, ColumnInfo col)>>();
        foreach (var kv in _results)
        {
            foreach (var col in kv.Value.Columns)
            {
                ...
                if (!enumRefs.TryGetValue(enumName, out var list))
                {
                    list = new List<(SheetParseResult, ColumnInfo)>();
                    enumRefs[enumName] = list;
                }
                list.Add((kv.Value, col));
            }
        }

        bool any = false;
        foreach (var kv in enumRefs)
        {
            ...
            foreach (var (sheet, col) in kv.Value)
            {
                int idx = sheet.Columns.IndexOf(col);
    ...
    private void GenerateSpecDataManager()
    {
        ...
        sb.AppendLine();
        foreach (var name in _results.Keys)
            if (name != "Config") sb.AppendLine($"        await Fetch{name}Async();");
        sb.AppendLine();
        ...
        foreach (var kv in _results)
        {
            ...
            sb.AppendLine($"    async UniTask Fetch{name}Async()");
            sb.AppendLine("    {");
            ...
            sb.AppendLine("        {");
            sb.AppendLine("            await req.SendWebRequest().ToUniTask();");
        ...
        sb.AppendLine("    }");
        sb.AppendLine("    static DateTime ParseDateTime(string s)");
        sb.AppendLine("    {");
        sb.AppendLine("        if (string.IsNullOrEmpty(s)) return default(DateTime);");
        sb.AppendLine("        return DateTime.Parse(s, System.Globalization.CultureInfo.InvariantCulture);");
        sb.AppendLine("    }");
    ...
    private static string BuildParseExpression(string cellExpr, string hint, string fieldName)
    {
        return hint switch
        {
            ...
            "string"   => cellExpr,
            "datetime" => $"ParseDateTime({cellExpr})",
