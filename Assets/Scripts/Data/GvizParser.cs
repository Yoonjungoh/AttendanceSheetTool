...
///   - 날짜 서식 컬럼은 "Date(2026,5,16)" 리터럴 문자열로 내려오며,
///     이 파서가 "yyyy-MM-ddTHH:mm:ss" ISO 형식으로 정규화함 (month는 0-indexed → +1 보정)
...
public static class GvizParser
{
    ...
    private static string ExtractV(string cell)
    {
        ...
        // 문자열
        if (first == '"')
            return ConvertGvizDateLiteral(DecodeJsonString(cell, vs + 1));
    ...
    // ── gviz 날짜 리터럴 "Date(y,m,d[,h,mi,s])" → ISO 문자열 변환 ──
    // gviz month는 0-indexed이므로 +1 보정. 패턴이 아니면 원본 그대로 반환.
    private static string ConvertGvizDateLiteral(string s)
    {
        if (string.IsNullOrEmpty(s) || !s.StartsWith("Date(") || !s.EndsWith(")"))
            return s;

        string inner = s.Substring(5, s.Length - 6);
        ...
        {
            if (!int.TryParse(parts[i].Trim(), out n[i]))
                return s;
        }

        try
        {
            var dt = new System.DateTime(n[0], n[1] + 1, n[2], n[3], n[4], n[5]);
